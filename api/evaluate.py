...
def analyze_image(img: Image):
    prompt = (
        ...
        f"2. Set of Equations like x^2 + 2x + 1 = 0, 3y + 4x = 0, 5x^2 + 6^y + 7 = 12, etc.: In this case, solve for the given variable, and the format should be a COMMA SEPARATED LIST OF DICTS, with dict 1 as {{'expr': 'x', 'result': 2, 'assign': True}} and dict 2 as {{'expr': 'y', 'result': 5, 'assign': True}}. This example assumes x was calculated as 2, and y as 5. Include as many dicts as number of variables. "
    ...
    # Passing image and prompt to get the output from gemini api
    response = client.models.generate_content(model="gemini-2.0-flash-exp", contents=[prompt, img])
    answers = []

    # Formatting the output
    print("Response\n", response.text)
    starting_index = response.text.find("json")
    json_str = response.text[starting_index:]
    cleaned_string = json_str.strip("```json\n").strip("\n```")

    # Seperate output as literals
    try:
        answers = ast.literal_eval(cleaned_string)
